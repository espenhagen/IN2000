...
class AirQualityRemoteDataSource {
    suspend fun fetchAirQualityAtPointDataSource(lat: String, lon: String): AirQualData? {
        ...
        val path =
            "https://in2000-apiproxy.ifi.uio.no/weatherapi/airqualityforecast/0.1/?lat=${lat}&lon=${lon}&filter_vars=AQI"
        ...
        return try {
            ...
            Log.d(
                "Response",
                "A network request exception was thrown in AirQualityDataSource: ${exception.message}"
            )
            null
        }
    }
